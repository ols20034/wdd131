...
/* ------------------------------------------------------------
   ...
   - Generates HTML for recipe card
   - Generates tags
...
function renderRating(rating) {
    ...
    return `
        <span class="rating" role="img" aria-label="Rating: ${rating} stars">
            ${"⭐".repeat(fullStars)}
            ${halfStar ? "☆" : ""}
        </span>
    `;
...
// Create tag HTML dynamically
function renderTags(tags) {
    return `<span class="recipe-type">${tags[0] || "Recipe"}</span>`;
}

// Full recipe card template
            ...
            <div class="recipe-text">
                ...
                <div class="recipe-info">
                    ...
                    <ul class="ingredients">
                        ${recipe.recipeIngredient.map(item => `<li>${item}</li>`).join("")}
                    </ul>
...
/* ------------------------------------------------------------
   2. DISPLAY FUNCTION  (Rubric: Code Quality)
------------------------------------------------------------ */

function displayRecipes(list) {
    const container = document.getElementById("recipeContainer");
    container.innerHTML = list.map(renderRecipe).join("");
}

/* ------------------------------------------------------------
   3. RANDOM RECIPE ON LOAD  (Rubric: Random Recipe)
------------------------------------------------------------ */
...
/* ------------------------------------------------------------
   4. FILTER + SORT RECIPES  (Rubric: Recipe Filtering)
------------------------------------------------------------ */
...
function filterRecipes(keyword) {
    ...
    const filtered = recipes.filter(recipe => {
        return (
            ...
            recipe.tags.join(" ").toLowerCase().includes(keyword)
        );
    });
...
/* ------------------------------------------------------------
   5. SEARCH INPUT LISTENER  (Rubric: Filtering)
------------------------------------------------------------ */
// CLICK SEARCH ICON TO RUN SEARCH
document.getElementById("searchButton").addEventListener("click", () => {
    ...
    if (keyword === "") {
        ...
        displayRecipes(filterRecipes(keyword));
    }
});


/* ------------------------------------------------------------
   6. INITIAL PAGE LOAD  (Rubric: Random Recipe)
------------------------------------------------------------ */
